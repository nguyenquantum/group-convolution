...
def matrix_entry(i, j, n, kernel_func):
    ...
    distance = np.sum(np.where(diff<1/2, diff, 1-diff))    ## Manhatten distance on a torus of size 1x1
...
def get_cyclic_product_reps(N):
    """ Return INVERSE (transpose) regular representations (permutation matrices of size N^2 x N^2) of the direct product of a cyclic group to itself, 
